 ...
 */
  ...
  #define THERMAL_PROTECTION_BED_PERIOD        35 // Seconds
...
#define ASSISTED_TRAMMING
  ...
  #define TRAMMING_POINT_XY { {  20, 20 }, { 256,  20 }, { 256, 260 }, { 20, 256 } }
...
// @section motion
/** * Input Shaping -- EXPERIMENTAL * * Zero Vibration (ZV) Input Shaping
 for X and/or Y movements.  * * This option uses a lot of SRAM for the step
 buffer.  The buffer size is * calculated automatically from
 SHAPING_FREQ_[XY], DEFAULT_AXIS_STEPS_PER_UNIT, * DEFAULT_MAX_FEEDRATE and
 ADAPTIVE_STEP_SMOOTHING.  The default calculation can * be overridden by
 setting SHAPING_MIN_FREQ and/or SHAPING_MAX_FEEDRATE.  * The higher the
 frequency and the lower the feedrate, the smaller the buffer.  * If the
 buffer is too small at runtime, input shaping will have reduced *
 effectiveness during high speed movements.  * * Tune with M593 D<factor>
 F<frequency>: * * D<factor> Set the zeta/ factor.  If axes (X, Y,
 etc.) are not specified, set for all axes.  * F<frequency> Set the
 frequency.  If axes (X, Y, etc.) are not specified, set for all axes.  *
 T[map] Input Shaping type, 0:ZV, 1:EI, 2:2H EI (not implemented yet) * X<1>
 Set the given parameters only for the X axis.  * Y<1> Set the given
 parameters only for the Y axis.  */
...
#define ADVANCED_PAUSE_FEATURE
  ...
  #define FILAMENT_CHANGE_UNLOAD_LENGTH      4  // (mm) The length of filament for a complete unload.
  ...
  #define FILAMENT_CHANGE_FAST_LOAD_LENGTH   4  // (mm) Load length of filament, from extruder gear to nozzle.
                                                  //   For Bowden, the full length of the tube and nozzle.
                                                  //   For direct drive, the full length of the nozzle.
  ...
  #define FILAMENT_UNLOAD_PURGE_RETRACT       14  // (mm) Unload initial retract length.
